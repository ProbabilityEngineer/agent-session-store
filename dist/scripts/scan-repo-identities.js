...
function id(...parts) { return createHash("sha256").update(parts.join("\0")).digest("hex").slice(0, 16); }
...
function normalizeGithubOrigin(url) {
    if (!url)
        return undefined;
...
async function git(cwd, args) { return (await execFileAsync("git", args, { cwd })).stdout.trim(); }
...
async function findRepos(root) {
    ...
    async function walk(dir, depth) {
        ...
        if (entries.some((e) => e.isDirectory() && (e.name === ".git" || e.name === ".jj"))) {
            out.push(dir);
            return;
        }
        for (const e of entries)
            if (e.isDirectory() && !["node_modules", "dist", "target", ".git", ".jj"].includes(e.name))
...
for (const root of roots)
    for (const path of await findRepos(root)) {
...
//# sourceMappingURL=scan-repo-identities.js.map