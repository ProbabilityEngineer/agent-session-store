...
function isRelocateEvidence(line) {
...
function buildSegments(session, lines) {
    ...
    if (!sortedEnds.includes(lines.length))
        sortedEnds.push(lines.length);
    const segments = [];
    const edges = [];
    let start = 1;
    for (const [segmentIndex, end] of sortedEnds.entries()) {
        ...
        const timestamps = segmentLines.map(parseEntryTimestamp).filter((ts) => Boolean(ts));
        const segmentDestinations = new Set();
        for (const [line, destinations] of destinationsByLine.entries()) {
            if (line >= start && line <= end)
                for (const dest of destinations)
                    segmentDestinations.add(dest);
        }
        segments.push({
            id,
            session,
...
async function readPrefixPairs() {
    ...
    try {
        ...
        const pairs = new Set();
        for (const item of parsed.bestCandidates ?? [])
            if (item.best?.source && item.best.destination)
                pairs.add(edgeKey(item.best.source, item.best.destination));
        for (const [, entries] of parsed.forkEntries ?? [])
            for (const entry of entries)
                if (entry.source && entry.destination)
                    pairs.add(edgeKey(entry.source, entry.destination));
        return pairs;
    }
    catch {
        return new Set();
...
async function birthtime(path) {
    try {
        return (await stat(path)).birthtime.toISOString();
    }
    catch {
        return undefined;
    }
}
function deltaSeconds(left, right) {
    if (!left || !right)
        return undefined;
...
async function main() {
    ...
    for (const segment of allSegments)
        bySession.set(segment.session, [...(bySession.get(segment.session) ?? []), segment]);
    const uniqueTails = [];
    ...
    const destinationBirthtimes = new Map();
    ...
    for (const edge of allEdges) {
        ...
        if (explicit)
            edge.class = "authoritativeManifest";
        else if (!edge.destinationExists || edge.isSelfEdge || isTruncatedPath(edge.destination) || edge.destinationsOnLine > 2 || edge.destinationsInSegment > 3)
            edge.class = "suppressedCopiedEvidence";
        else if (prefixBacked)
            edge.class = "prefixBackedForensicEvidence";
        else if (segment?.class === "unique")
            edge.class = "forensicSegmentEvidence";
        else if ((segment?.repeatedInFiles ?? 0) > 3)
            edge.class = "suppressedCopiedEvidence";
        else
            edge.class = "duplicatedForensicEvidence";
        let score = 0;
        const reasons = [];
        if (explicit) {
            score += 100;
        ...
        if (edge.destinationExists) {
            score += 20;
            reasons.push("destination-exists");
        }
        else {
            score -= 50;
            reasons.push("missing-or-truncated-destination");
        }
        if (!edge.isSelfEdge) {
            score += 10;
            reasons.push("not-self");
        }
        else {
            score -= 30;
            reasons.push("self-edge");
        }
        if (segment?.class === "unique") {
            score += 10;
            reasons.push("unique-segment");
        }
        if (edge.destinationsOnLine <= 1 && edge.destinationsInSegment <= 1) {
            score += 10;
            reasons.push("single-destination-evidence");
        }
        if (edge.destinationBirthDeltaSeconds !== undefined && Math.abs(edge.destinationBirthDeltaSeconds) <= 300) {
            score += 10;
            reasons.push("near-destination-birthtime");
        }
    ...
    const bestByDestination = [...rankedEdges.reduce((acc, ranked) => {
            const current = acc.get(ranked.edge.destination);
            if (!current || ranked.score > current.score)
                acc.set(ranked.edge.destination, ranked);
            return acc;
    ...
    const classCounts = allEdges.reduce((acc, edge) => {
...
//# sourceMappingURL=index-session-segments.js.map