...
async function main() {
    ...
    const validations = manifest.map((record, index) => {
        ...
        const warnings = [];
        if (!source)
            warnings.push("missing source file");
        if (!dest)
            warnings.push("missing destination file");
        if (destNameDelta !== undefined && Math.abs(destNameDelta) > 300)
            warnings.push("manifest ts far from destination filename ts");
        if (destBirthDelta !== undefined && Math.abs(destBirthDelta) > 300)
            warnings.push("manifest ts far from destination birthtime");
...
//# sourceMappingURL=validate-session-timeline.js.map