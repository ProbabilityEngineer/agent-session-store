...
const graphInputDir = join(agentDir, "session-graph");
const outputDir = process.env.AGENT_SESSION_GRAPH_OUTPUT_DIR ?? join(home, "Desktop", "session-graphs");
const manifestPath = join(agentDir, "relocations.jsonl");
const overlayPath = join(graphInputDir, "lineage-overlays.jsonl");
...
function cwdDisplay(cwd) {
    if (cwd.startsWith(`${home}/git/`))
        return cwd.slice(`${home}/git/`.length);
    if (cwd.startsWith(`${home}/`))
        return cwd.slice(`${home}/`.length);
    return cwd;
}
function label(cwd, session) {
    if (cwd && !cwd.startsWith("("))
        return cwdDisplay(cwd);
...
function labelFor(report, session, cwd) {
    if (cwd && !cwd.startsWith("("))
        return label(cwd, session);
    const curated = report.sessionLabelsBySession[session];
    if (curated?.label)
        return curated.label;
    if (curated?.cwd)
        return label(curated.cwd, session);
    if (basename(session).includes("_relocated_"))
        return bucketLabel(session) ?? label(undefined, session);
    return label(report.sessionStats[session]?.cwd, session);
}
function isTempSession(path) {
    ...
    try {
        ...
        for (const line of raw.split("\n")) {
            ...
            try {
                ...
                if (candidateCwd && (!cwd || candidateCwd.length > cwd.length))
                    cwd = candidateCwd;
...
async function linesAt(path, eventTs) {
    try {
        const raw = await readFile(path, "utf8");
        let lines = 0;
        let lastTimestamp;
...
async function build() {
    ...
    const overlayEdges = overlays.filter((record) => record.kind === "edge");
    ...
    const statsEntries = await Promise.all([...sessions].map(async (path) => [path, await sessionStats(path)]));
...
function selectedStarts(report, options = {}) {
    ...
    const bucketImportantPaths = new Set();
    const startsByBucket = new Map();
    for (const start of report.sessionStarts) {
        const bucket = bucketLabel(start.path);
        if (!bucket || connectedSessions.has(start.path) || isTempSession(start.path))
            continue;
    ...
    for (const [bucket, starts] of startsByBucket) {
        if ((bucketCounts.get(bucket) ?? 0) < 5)
            continue;
        for (const start of starts.sort((a, b) => b.currentLines - a.currentLines || b.ts.localeCompare(a.ts)).slice(0, 3))
            bucketImportantPaths.add(start.path);
    }
    function importantStandalone(start) {
        if (connectedSessions.has(start.path) || isTempSession(start.path))
            return false;
...
function mermaid(report, options = {}) {
    const lines = ["flowchart LR"];
    const sessionIds = new Map();
    function sessionNode(path, cwd, currentLines) {
    ...
    for (const [source, sourceEdges] of edgesBySource) {
        ...
        for (const edge of sourceEdges) {
            ...
            if (previousState)
                lines.push(`  ${previousState} -. later .-> ${stateId}`);
    ...
    for (const start of starts)
        lines.push(`  class start_${shortHash(start.path)} start;`);
    for (const id of sessionIds.values())
        lines.push(`  class ${id} session;`);
    for (const edge of edges)
        lines.push(`  class s_${shortHash(`${edge.sourceSession}:${edge.ts}:${edge.id}`)} state;`);
    return lines.join("\n");
}
function html(report, mmd, stamp) {
    return `<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>${stamp} — Lineage Full</title>
...
function timelineData(report, options = {}) {
    const starts = selectedStarts(report, options);
    const sessionPaths = new Set();
    for (const start of starts)
        sessionPaths.add(start.path);
    const edges = visibleEdges(report, options);
    for (const edge of edges) {
...
function projectKey(report, path, cwd) {
    return labelFor(report, path, cwd);
}
function temporalInventoryJson(report) {
    return {
...
function temporalTimelineJson(report, options = {}) {
...
function focusedMermaid(report, options = {}) {
    const lines = ["flowchart LR"];
    const sessionIds = new Map();
    const firstEdgeBySession = new Map();
    for (const edge of visibleEdges(report, options)) {
        for (const path of [edge.sourceSession, edge.destinationSession]) {
            const existing = firstEdgeBySession.get(path);
            if (!existing || edge.ts < existing)
                firstEdgeBySession.set(path, edge.ts);
        }
    }
    function sessionNode(path, cwd, currentLines) {
        const existing = sessionIds.get(path);
...
function focusedHtml(report, mmd, stamp) {
    return `<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>${stamp} — Lineage Focused</title>
<script src="https://cdn.jsdelivr.net/npm/svg-pan-zoom@3.6.2/dist/svg-pan-zoom.min.js"></script>
<script type="module">
import mermaid from 'https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.esm.min.mjs';
mermaid.initialize({ startOnLoad: false, securityLevel: 'loose' });
...
function temporalTimelineHtml(report, options = {}) {
    const data = timelineData(report, options);
    const left = 280;
    const right = 80;
    const top = 50;
    ...
    const rowFor = (path, cwd) => options.groupBy === "session" ? path : projectKey(report, path, cwd);
    const x = (ts) => left + ((Date.parse(ts) - data.minTime) / span) * (width - left - right);
    const tickCount = 10;
    ...
    svg.push(`</svg>`);
    return `<!doctype html>
<html><head><meta charset="utf-8"><title>${options.stamp ? `${options.stamp} — ` : ""}${options.groupBy === "session" ? "Timeline Sessions" : "Timeline Projects"}</title>
<script src="https://cdn.jsdelivr.net/npm/svg-pan-zoom@3.6.2/dist/svg-pan-zoom.min.js"></script>
...
async function main() {
    ...
    const htmlDoc = html(report, mmd, stamp);
    const focusedMmd = focusedMermaid(report, { includeUnresolved });
    const focusedDoc = focusedHtml(report, focusedMmd, stamp);
    const timelineJson = JSON.stringify(temporalTimelineJson(report, { allStarts, includeUnresolved, groupBy: "project" }), null, 2) + "\n";
    const timelineHtml = temporalTimelineHtml(report, { allStarts, includeUnresolved, groupBy: "project", stamp });
    ...
    for (const path of written)
        console.log(path);
    if (!includeData)
        console.log("Data/debug artifacts omitted; rerun with --include-data to write JSON/MMD/MD files.");
    if (snapshot)
        console.log("--snapshot is no longer needed: build-graphs outputs are timestamped by default.");
}
await main();
//# sourceMappingURL=build-graphs.js.map