...
try {
    ...
    const activeTimeMetrics = db.prepare("SELECT id, kind, path, generated_at, generator, metadata_json FROM artifacts WHERE kind = 'active_time_metric' ORDER BY id").all().map((row) => {
        const metadata = parseJson(row.metadata_json, {});
    ...
    const temporalActivitySpans = sessions.flatMap((session) => {
        ...
        const activeTime = typeof session.metadata?.activeTime === "object" && session.metadata.activeTime ? session.metadata.activeTime : undefined;
        const visitRows = typeof session.metadata?.visitRowMetrics === "object" && session.metadata.visitRowMetrics ? session.metadata.visitRowMetrics.visitRows : undefined;
