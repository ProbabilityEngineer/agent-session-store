...
type EvidenceRow = { id: string; summary: string; data_json: string };

function count(db: DatabaseSync, sql: string) { return (db.prepare(sql).get() as CountRow).c; }
function all<T>(db: DatabaseSync, sql: string) { return db.prepare(sql).all() as T[]; }
...
try {
	...
	const backupPaths = all<PathRow>(db, "SELECT path FROM session_observations WHERE path LIKE '%/session-backups/%' ORDER BY path").map((row) => row.path);
	const evidence = all<EvidenceRow>(db, "SELECT id, summary, data_json FROM evidence WHERE kind = 'backup_presence' OR data_json LIKE '%session-backups%' ORDER BY id");
	const remainingRawDependencies = all<PathRow>(db, "SELECT path FROM artifacts WHERE path LIKE '%/session-backups/%' UNION SELECT uri AS path FROM sources WHERE uri LIKE '%/session-backups/%' ORDER BY path").map((row) => row.path);
