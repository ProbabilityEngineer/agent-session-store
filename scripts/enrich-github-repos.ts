...
type Row = { kind?: string; stableName?: string; metadata?: any; remoteUrl?: string } & Record<string, unknown>;
function githubFromRow(row: Row) {
	const gh = row.metadata?.github;
	if (gh?.owner && gh?.name) return { owner: String(gh.owner), name: String(gh.name) };
...
async function readRows(): Promise<Row[]> { try { return (await readFile(sidecar, "utf8")).split("\n").filter(Boolean).map((l) => JSON.parse(l)); } catch { return []; } }
if (!token) {
...
for (const row of rows) {
	if (row.kind !== "repo-observation") continue;
	const gh = githubFromRow(row);
	if (!gh || !row.stableName) continue;
