...
try {
	const identities = db.prepare("SELECT id, stable_name, display_name, description, confidence, source FROM repo_identities ORDER BY stable_name").all() as any[];
	const observations = db.prepare("SELECT repo_identity_id, path, bucket, remote_url, valid_from, valid_to, confidence, source, metadata_json FROM repo_observations ORDER BY repo_identity_id, valid_from, path").all() as any[];
	const events = db.prepare("SELECT event_type, repo_identity_id, related_repo_identity_id, from_path, to_path, timestamp, confidence, source, manual_review_required, summary, metadata_json FROM repo_events ORDER BY timestamp, event_type").all() as any[];
	...
	for (const identity of identities) {
		...
		for (const row of obs) lines.push(`- ${row.path ?? row.bucket ?? row.remote_url ?? "(unknown)"}${row.valid_from || row.valid_to ? ` (${row.valid_from ?? "?"} → ${row.valid_to ?? "?"})` : ""} [${row.confidence}]`);
		...
		for (const row of evs) lines.push(`- ${row.timestamp ?? "unknown"} ${row.event_type}: ${row.summary ?? `${row.from_path ?? ""} -> ${row.to_path ?? ""}`.trim()} [${row.confidence}${row.manual_review_required ? ", review" : ""}]`);
		lines.push("");
	}
	if (!identities.length) lines.push("No repo identities imported yet. Add records to `~/.pi/agent/session-store/repo-identities.jsonl` and run `npm run build-store`.", "");
