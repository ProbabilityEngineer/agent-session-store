...
type Event = { id: string; sessionId: string; observationId?: string; sourceId: string; provider: string; providerEventId?: string; eventType: string; timestamp?: string; ordinal: number; rowNumber?: number; role?: string; toolName?: string; byteOffset?: number; byteCount?: number; contentSha256?: string; metadata?: Json };
...
function rowCwd(row: Json): string | undefined { const cwd = typeof row.cwd === "string" ? row.cwd : undefined; const session = row.session as Json | undefined; const value = cwd ?? (typeof session?.cwd === "string" ? session.cwd : undefined); return value ? normalizeRepoPath(value) : undefined; }
...
function rowRole(row: Json): string | undefined { return str(row.role) ?? str(asObj(row.message)?.role) ?? str(asObj(row.payload)?.role); }
function rowToolName(row: Json): string | undefined { return str(row.toolName) ?? str(row.tool_name) ?? str(asObj(row.message)?.toolName) ?? str(asObj(row.payload)?.toolName); }
function rowProviderEventId(row: Json): string | undefined { return str(row.id) ?? str(row.eventId) ?? str(row.event_id) ?? str(asObj(row.payload)?.id); }
...
async function sessionObservation(path: string, sourceId: string): Promise<{ session: Session; observation: SessionObservation; events: Event[] }> {
	...
	if (await exists(path)) {
		...
		for (const lineWithMaybeEmpty of raw.split("\n")) {
			...
			if (!line.trim()) { byteOffset += byteCount + 1; continue; }
			lineCount++;
			try {
				const row = JSON.parse(line) as Json;
				const ts = rowTimestamp(row);
				if (ts) timestampedRows++;
...
type RawSourceManifestEntry = { path: string; provider?: string; kind: string; status: "imported" | "artifact" | "source-root"; size?: number; mtime?: string; sha256?: string; sessionId?: string; observationId?: string; artifactId?: string; sourceId?: string; metadata?: Json };

async function rawSourceManifest(store: Store): Promise<RawSourceManifestEntry[]> {
	const entries: RawSourceManifestEntry[] = [];
	for (const source of store.sources) entries.push({ path: source.uri, provider: source.provider, kind: source.kind, status: "source-root", sourceId: source.id, metadata: source.metadata });
	for (const observation of store.sessionObservations) entries.push({ path: observation.path, provider: store.sessions.find((session) => session.id === observation.sessionId)?.provider, kind: "session_observation", status: "imported", size: observation.fileSize, mtime: observation.fileMtime, sha256: observation.contentSha256, sessionId: observation.sessionId, observationId: observation.id, sourceId: observation.sourceId, metadata: { providerSessionId: observation.providerSessionId, lineCount: observation.lineCount } });
	for (const artifact of store.artifacts) entries.push({ path: artifact.path, kind: artifact.kind, status: "artifact", sha256: artifact.inputHash, artifactId: artifact.id, metadata: artifact.metadata });
	return entries.sort((a, b) => a.path.localeCompare(b.path) || a.kind.localeCompare(b.kind));
}

function rawSourceManifestMarkdown(entries: RawSourceManifestEntry[]) {
	const byStatus = entries.reduce<Record<string, number>>((acc, entry) => { acc[entry.status] = (acc[entry.status] ?? 0) + 1; return acc; }, {});
	const byProvider = entries.reduce<Record<string, number>>((acc, entry) => { const key = entry.provider ?? "(none)"; acc[key] = (acc[key] ?? 0) + 1; return acc; }, {});
...
function rowAtOrBefore(events: Event[], timestamp?: string) {
	if (!timestamp) return undefined;
	let row: number | undefined;
	for (const event of events) if (event.timestamp && event.rowNumber && event.timestamp <= timestamp) row = Math.max(row ?? 0, event.rowNumber);
	return row;
}
...
function deriveVisitRowMetrics(store: Store) {
	...
	for (const list of eventsBySession.values()) list.sort((a, b) => (a.timestamp ?? "").localeCompare(b.timestamp ?? "") || a.ordinal - b.ordinal);
	for (const edge of store.edges) {
		...
		if (sourceRowAtMove == null && targetRowAtMove == null) continue;
	...
	const incomingBySession = new Map<string, Edge[]>();
	const outgoingBySession = new Map<string, Edge[]>();
	for (const edge of store.edges) {
		if (edge.sourceSessionId === edge.targetSessionId) continue;
		const incoming = incomingBySession.get(edge.targetSessionId) ?? [];
		incoming.push(edge); incomingBySession.set(edge.targetSessionId, incoming);
		const outgoing = outgoingBySession.get(edge.sourceSessionId) ?? [];
		outgoing.push(edge); outgoingBySession.set(edge.sourceSessionId, outgoing);
	}
	for (const session of store.sessions) {
		...
		if (arrivalRow == null && departureRow == null) continue;
...
function deriveActiveTimeMetrics(store: Store, seen: Seen, generatedAt: string) {
	...
	for (const session of store.sessions) {
		...
		if (cwd) agg.paths.add(cwd);
		agg.providers.add(session.provider);
		if (!asObj(metadata.activeTime) && (session.lineCount ?? 0) > 1) agg.intervals.push({ activeMinutes: 0, workBlockCount: 0, sessionIds: [session.id], excluded: true, warnings: ["session lacks timestamped event rows; active time is incomplete"] });
		byProject.set(project, agg);
	}
...
function externalEventsFromRaw(provider: string, sourceId: string, sessionId: string, observationId: string, raw: string) {
	const events: Event[] = [];
	let rowNumber = 0;
	let byteOffset = 0;
	for (const line of raw.split("\n")) {
		...
		try {
			...
			if (!timestamp) { byteOffset += byteCount + 1; continue; }
...
async function addExternalProviderSessions(store: Store, seen: { sources: Set<string>; sessions: Set<string>; obs: Set<string>; events: Set<string>; labels: Set<string>; artifacts: Set<string> }, addSource: (provider: string, kind: string, uri: string, label?: string, metadata?: Json) => string) {
	function addParsed(provider: string, sourceId: string, path: string, providerSessionId: string, meta: Parameters<typeof externalSession>[4], raw?: string) {
		...
		if (raw) for (const event of externalEventsFromRaw(provider, sourceId, session.id, observation.id, raw)) pushUnique(store.events, seen.events, event);
		...
		if (typeof session.metadata?.cwd === "string") pushUnique(store.labels, seen.labels, { id: id("label", provider, session.id, "cwd", session.metadata.cwd), targetType: "session", targetId: session.id, labelType: "cwd", value: session.metadata.cwd, confidence: "observed", sourceId, metadata: { originalCwd: meta.cwd } });
	}
	for (const root of codingSessionsRoots) {
		...
		for (const path of await walkFiles(codexRoot, (p) => p.endsWith(".jsonl"))) {
			...
			let cwd: string | undefined; let title: string | undefined; let start: string | undefined; let end: string | undefined; let timestampedRows = 0; const counts: Record<string, number> = {};
			for (const line of fs.raw.split("\n")) { if (!line.trim()) continue; try { const row = JSON.parse(line) as Json; counts[eventType(row)] = (counts[eventType(row)] ?? 0) + 1; const ts = rowTimestamp(row) ?? str(asObj(row.payload)?.timestamp); if (ts) timestampedRows++; start = minTs(start, ts); end = maxTs(end, ts); const payload = asObj(row.payload); cwd ??= str(payload?.cwd) ?? str(asObj(payload?.turn_context)?.cwd); title ??= str(payload?.title); sid = str(payload?.id) ?? sid; } catch {} }
		...
		for (const path of await walkFiles(ompRoot, (p) => p.endsWith(".jsonl"))) {
			...
			let cwd: string | undefined; let title: string | undefined; let start: string | undefined; let end: string | undefined; let timestampedRows = 0; const counts: Record<string, number> = {};
			for (const line of fs.raw.split("\n")) { if (!line.trim()) continue; try { const row = JSON.parse(line) as Json; counts[eventType(row)] = (counts[eventType(row)] ?? 0) + 1; const ts = rowTimestamp(row); if (ts) timestampedRows++; start = minTs(start, ts); end = maxTs(end, ts); if (row.type === "session") { sid = str(row.id) ?? sid; cwd ??= str(row.cwd); title ??= str(row.title); } } catch {} }
		...
		for (const path of await walkFiles(factoryRoot, (p) => p.endsWith(".jsonl"))) {
			const fs = await fileStats(path).catch(() => undefined); if (!fs) continue;
			let sid = basename(path).replace(/\.jsonl$/, ""); let cwd: string | undefined; let title: string | undefined; let start: string | undefined; let end: string | undefined; let timestampedRows = 0; const counts: Record<string, number> = {};
			for (const line of fs.raw.split("\n")) { if (!line.trim()) continue; try { const row = JSON.parse(line) as Json; counts[eventType(row)] = (counts[eventType(row)] ?? 0) + 1; const ts = rowTimestamp(row); if (ts) timestampedRows++; start = minTs(start, ts); end = maxTs(end, ts); if (row.type === "session_start") { sid = str(row.id) ?? sid; cwd ??= str(row.cwd); title ??= str(row.sessionTitle) ?? str(row.title); } } catch {} }
		...
		for (const path of await walkFiles(join(claudeRoot, "transcripts"), (p) => p.endsWith(".jsonl"))) {
			const fs = await fileStats(path).catch(() => undefined); if (!fs) continue;
			const sid = basename(path).replace(/\.jsonl$/, ""); let cwd: string | undefined; let title: string | undefined; let start: string | undefined; let end: string | undefined; let timestampedRows = 0; const counts: Record<string, number> = {};
			for (const line of fs.raw.split("\n")) { if (!line.trim()) continue; try { const row = JSON.parse(line) as Json; counts[eventType(row)] = (counts[eventType(row)] ?? 0) + 1; const ts = rowTimestamp(row); if (ts) timestampedRows++; start = minTs(start, ts); end = maxTs(end, ts); cwd ??= rowCwd(row) ?? str(asObj(row.tool_input)?.path); title ??= str(row.summary); } catch {} }
		...
		for (const path of await walkFiles(lateRoot, (p) => /^session-.*\.json$/.test(basename(p)) && !p.endsWith(".meta.json"))) {
			const fs = await fileStats(path).catch(() => undefined); if (!fs) continue;
			const rows = JSON.parse(fs.raw) as Json[]; const meta = await readJson<Json>(path.replace(/\.json$/, ".meta.json")); const counts: Record<string, number> = {}; let timestampedRows = 0;
			for (const row of Array.isArray(rows) ? rows : []) { counts[str(row.role) ?? "unknown"] = (counts[str(row.role) ?? "unknown"] ?? 0) + 1; if (rowTimestamp(row)) timestampedRows++; }
...
function deriveAdditionalMetadata(store: Store, seen: Seen, generatedAt: string) {
	...
	const byStableName = new Map<string, RepoIdentity[]>();
	for (const repo of store.repoIdentities) { const list = byStableName.get(repo.stableName) ?? []; list.push(repo); byStableName.set(repo.stableName, list); }
	const canonicalRepoIdentityId = new Map<string, string>();
	for (const group of byStableName.values()) {
		...
		for (const repo of group) canonicalRepoIdentityId.set(repo.id, chosen.id);
	}
	const canonicalRid = (repoIdentityId: string) => canonicalRepoIdentityId.get(repoIdentityId) ?? repoIdentityId;
	const curatedRank = (repoIdentityId: string) => repoIdentityById.get(canonicalRid(repoIdentityId))?.stableName.startsWith("project:") ? 2 : repoIdentityById.get(canonicalRid(repoIdentityId))?.confidence === "manual" ? 1 : 0;
...
function replaceSqlite(dbPath: string, store: Store) {
	...
	try {
		...
		for (const r of store.events) eventStmt.run(r.id, r.sessionId, r.observationId ?? null, r.sourceId, r.provider, r.providerEventId ?? null, r.eventType, r.timestamp ?? null, r.ordinal, r.rowNumber ?? null, r.role ?? null, r.toolName ?? null, r.byteOffset ?? null, r.byteCount ?? null, r.contentSha256 ?? null, json(r.metadata));
...
async function main() {
	...
	const seen = { sources: new Set<string>(), sessions: new Set<string>(), obs: new Set<string>(), events: new Set<string>(), edges: new Set<string>(), labels: new Set<string>(), aliases: new Set<string>(), classes: new Set<string>(), evidence: new Set<string>(), backups: new Set<string>(), repos: new Set<string>(), artifacts: new Set<string>(), marks: new Set<string>(), batches: new Set<string>(), repoIdentities: new Set<string>(), repoObservations: new Set<string>(), repoEvents: new Set<string>() };
	...
	for (const path of [...paths].sort()) {
		...
		pushUnique(store.sessions, seen.sessions, session);
		pushUnique(store.sessionObservations, seen.obs, observation);
		for (const event of events) pushUnique(store.events, seen.events, event);
	...
	const exportStore = { ...store, events: [] as Event[], eventIndex: { storage: "sqlite", table: "events", count: store.events.length, privacyStatus: "metadata-only" } };
