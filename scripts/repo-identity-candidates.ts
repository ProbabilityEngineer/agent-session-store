...
type Session = { id: string; provider: string; startTimestamp?: string; endTimestamp?: string; firstSeenAt?: string; lastSeenAt?: string; metadata?: Json };
type RepoObservation = { repoIdentityId: string; path?: string; remoteUrl?: string; confidence?: string };
type Store = { sessions?: Session[]; repoObservations?: RepoObservation[] };

type PathInfo = { path: string; normalizedName: string; parent: string; sessions: number; first?: string; last?: string; providers: string[]; remote?: string; existingIdentityIds: string[] };
type Candidate = { id: string; suggestedStableName: string; displayName: string; paths: string[]; confidence: "high" | "medium" | "low"; manualApprovalRequired: boolean; evidence: string[]; metrics: { sessions: number; providers: string[]; first?: string; last?: string } };
...
function displayName(path: string) { return basename(path).replace(/[-_]+/g, " ").replace(/\bv(\d+)$/i, "v$1").replace(/\b\w/g, (c) => c.toUpperCase()); }
function stableName(path: string) { return basename(path).toLowerCase().replace(/([a-z])([A-Z])/g, "$1-$2").replace(/[_\s]+/g, "-").replace(/[^a-z0-9-]/g, "").replace(/-+/g, "-"); }
function acronymBase(name: string) { return name.replace(/v\d+$/i, "").split(/[-_\s]+/).filter(Boolean).map((part) => part[0]).join("").toLowerCase(); }
function compactWithoutVersion(name: string) { return name.toLowerCase().replace(/v\d+$/i, "").replace(/[^a-z0-9]/g, ""); }
async function gitRemote(path: string) {
	if (!existsSync(path)) return undefined;
	try { return (await execFileAsync("git", ["remote", "get-url", "origin"], { cwd: path })).stdout.trim() || undefined; } catch { return undefined; }
}
...
function withinDays(a?: string, b?: string, days = 14) {
	if (!a || !b) return false;
	const gap = Math.abs(Date.parse(a) - Date.parse(b));
	return Number.isFinite(gap) && gap <= days * 86400000;
}

const store = JSON.parse(await readFile(exportPath, "utf8")) as Store;
...
for (const session of store.sessions ?? []) {
	...
	if (!item.providers.includes(session.provider)) item.providers.push(session.provider);
	byPath.set(path, item);
}
for (const obs of store.repoObservations ?? []) {
	if (!obs.path) continue;
	const path = normalizeRepoPath(obs.path);
	const item = byPath.get(path) ?? { path, normalizedName: normalizeName(path), parent: dirname(path), sessions: 0, providers: [], existingIdentityIds: [] };
	if (obs.remoteUrl) item.remote = obs.remoteUrl;
	if (!item.existingIdentityIds.includes(obs.repoIdentityId)) item.existingIdentityIds.push(obs.repoIdentityId);
	byPath.set(path, item);
}
...
for (let i = 0; i < paths.length; i++) {
	for (let j = i + 1; j < paths.length; j++) {
		...
		if (a.remote && b.remote && a.remote === b.remote) evidence.push(`same git remote: ${a.remote}`);
		if (a.parent === b.parent) evidence.push(`same parent directory: ${a.parent}`);
		else if (basename(a.parent).toLowerCase() === basename(b.parent).toLowerCase()) evidence.push(`same parent project folder name: ${basename(a.parent)}`);
		if (a.normalizedName === b.normalizedName) evidence.push(`same normalized basename: ${a.normalizedName}`);
		else if (a.normalizedName.includes(b.normalizedName) || b.normalizedName.includes(a.normalizedName)) evidence.push(`basename containment: ${basename(a.path)} ↔ ${basename(b.path)}`);
		else if (nameSimilarity(a.normalizedName, b.normalizedName) >= 0.72) evidence.push(`similar basenames: ${basename(a.path)} ↔ ${basename(b.path)}`);
		...
		if ((aAcronym && aAcronym === bCompact) || (bAcronym && bAcronym === aCompact)) evidence.push(`acronym/name match: ${basename(a.path)} ↔ ${basename(b.path)}`);
		if (withinDays(a.last, b.first, 45) || withinDays(b.last, a.first, 45)) evidence.push("temporal continuity within 45 days");
...
for (const c of out.candidates) md.push(`## ${c.displayName}`, ``, `Stable name: \`${c.suggestedStableName}\``, `Confidence: ${c.confidence}`, `Manual approval required: ${c.manualApprovalRequired}`, `Sessions: ${c.metrics.sessions}`, `Providers: ${c.metrics.providers.join(", ")}`, `First/last: ${c.metrics.first ?? "?"} → ${c.metrics.last ?? "?"}`, ``, `Paths:`, ...c.paths.map((p) => `- \`${p}\``), ``, `Evidence:`, ...c.evidence.map((e) => `- ${e}`), ``);
